...
export_alerting() {
    ...
    if body=$(curl -fsS "$GRAFANA_URL$path" 2>/dev/null); then
        # Reject exports that contain empty-uid entries. Grafana's contact
        # points endpoint returns its internally-managed default receiver
        # with `uid: ""`, and re-provisioning that fails the provisioner
        # with "no uid is set", breaking Grafana boot on the next restart.
        # Users who haven't created anything get the empty stub; users with
        # real config get their config.
        if echo "$body" | grep -qE 'uid: *""'; then
            printf 'apiVersion: 1\n' > "$outfile"
            echo "   $(basename "$outfile")  ($label — nothing user-defined; stub written)"
        else
            echo "$body" > "$outfile"
